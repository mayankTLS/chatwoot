...
export const maskEmail = (email, options = {}) => {
  ...
  switch (pattern) {
    ...
    case 'standard': {
      ...
      const maskedDomain = preserveDomain
        ? domain
        : `***${domain.split('.').pop()}`;
...
export const maskPhone = (phone, options = {}) => {
  ...
  switch (pattern) {
    ...
    case 'standard': {
      ...
      if (phone.includes('+')) {
        const cc = preserveCountryCode
          ? phone.substring(0, phone.indexOf(' ') || 3)
          : '+**';
...
export const shouldMaskData = (
  ...
  if (
    ...
    accountSettings?.masking_rules?.admin_bypass === true
  ) {
    return false;
  }
 ...
 */
export const getMaskingPattern = (
  currentUser,
  accountSettings,
  dataType = 'email'
) => {
 ...
 */
export const createRevealHandler = (
  originalValue,
  maskedValue,
  revealDuration = 3000
) => {
